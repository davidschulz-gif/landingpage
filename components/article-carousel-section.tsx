            ...
            {articleData.map((article, index) => (
              <CarouselItem key={index} className="pl-2 md:pl-4 md:basis-1/2 lg:basis-1/3">
                <motion.div
                  ...
                  viewport={{ once: true }}
                >
                  <Card className="border-0 gap-0 py-0 mb-4 ml-4 shadow-lg hover:shadow-xl transition-shadow duration-300 bg-white">
                    <CardContent className="p-0">
                      ...
                      <div className="p-6">
                        <BreathingAnimationText animationType="black-gray">
                          <h3 className="text-lg uppercase font-semibold text-gray-900 mb-3 line-clamp-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                            {article.title}
                          </h3>
                        </BreathingAnimationText>
                        ...
                        {article.title === "GOOGLE REVIEWS" && (
                          <div className="flex items-center mb-3">
                            <div className="flex items-center">
                              {[1, 2, 3, 4, 5].map((star) => (
                                <svg
                                  key={star}
                                  className={`w-5 h-5 ${
                                    star <= 4 ? 'text-yellow-400' : star === 5 ? 'text-gray-300' : 'text-gray-300'
                                  }`}
                                  fill={star <= 4 || (star === 5 && star <= 4.5) ? 'currentColor' : 'none'}
                                  viewBox="0 0 24 24"
                                  stroke="currentColor"
                                >
                                  {star === 5 ? (
                              ...
                              <svg className="w-5 h-5 text-yellow-400 absolute" style={{ marginLeft: '80px' }}>
                                <defs>
                            ...
                            <BreathingAnimationText animationType="black-gray">
                              <span className="ml-2 text-sm font-medium text-gray-700">4.5/5</span>
                            </BreathingAnimationText>
                          </div>
                        )}
                        <BreathingAnimationText animationType="black-gray">
