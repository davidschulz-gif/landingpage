...
import { Monitor, Grid, Info, Sparkles, ExternalLink } from 'lucide-react'
import { appUrl } from '@/lib/constants'

export function ViewerShowcase() {
  const locale = useLocale()
  const t = useTranslations('ResearchPage.viewers')
  const [activeTab, setActiveTab] = useState<'catalog' | 'flyout' | 'product-page' | 'texture-assignment'>('catalog')
  ...
  const getIframeSrc = () => {
    switch (activeTab) {
      ...
      case 'texture-assignment':
        return `${appUrl}/create?textures=open`
      default:
        return `${appUrl}/create?catalog=open`
    }
  }
    ...
    <div className="w-full space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-3 rounded-2xl bg-neutral-950 border border-neutral-800 text-white shadow-2xl">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            onClick={() => setActiveTab('texture-assignment')}
            className={`px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center gap-2 transition ${
              activeTab === 'texture-assignment'
                ? 'bg-blue-600 text-white shadow-md'
                : 'text-neutral-400 hover:text-white hover:bg-neutral-800'
            }`}
          >
      ...
      <div className="w-full relative rounded-2xl overflow-hidden border border-neutral-300 dark:border-neutral-800 bg-white shadow-2xl">
        <iframe
          src={iframeSrc}
          key={activeTab}
          className="w-full h-[780px] md:h-[880px] border-0 rounded-2xl bg-white shadow-inner transition-all duration-300"
          title={t('iframeTitle')}
        />
      </div>
